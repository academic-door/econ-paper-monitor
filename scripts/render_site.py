...
def secondary_page_lede(title: str) -> str:
    ...
    if "归档" in title:
        return "按本站本站首次发现日期组织的每日记录，官方发布日期与本站首次发现日期分开显示。"
    if "工作论文" in title:
...
def working_papers_body(records: list[dict[str, Any]], *, view: str = "all") -> str:
    ...
    note = "覆盖工作论文与机构研究来源，按本站首次发现时间倒序排列；官方日期与本站本站首次发现日期分开显示。"
    tabs = [
...
def recent72_body(records: list[dict[str, Any]]) -> str:
    ...
    return f"""<section class="section-head">
  <div><h2>最近72小时</h2><p>按本站本站首次发现日期展示今天及前两天记录，适合连续查看近 3 天选题池。</p></div>
...
def main() -> None:
    ...
    for daily_date, daily_records in sorted(by_date.items(), reverse=True):
        ...
        body = (
            ...
            f'<p>本站本站首次发现日期：{html_escape(daily_date)}；官方/在线日期范围：{html_escape(official_summary)}。支持按期刊、主题、日期类型、可信度和“与中国相关”筛选。</p></div></section>'
